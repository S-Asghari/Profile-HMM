...
def Viterbi(seq, A, B, pi, T, T2):

    delta = np.zeros(shape=(3*T+3, T2), dtype=float)      # δ
    psi = np.zeros(shape=(3*T+3, T2), dtype=float)        # ψ
        ...
        for j in range(0, 3*T+3):
            ...
            for i in range(3*T+3):
                if delta[i][t-1] * A[i][j] > maxx:
                    ...
                    psi[j][t] = i
            if j%3 == 0 and j != 0:     # D state
                delta[j][t] = maxx
            else:
                delta[j][t] = maxx * B[j][int(seq[t])]

    q_star = np.zeros(T2, dtype=int)

    maxx = 0
    for i in range(3*T+3):
        if delta[i][T2-1] > maxx:
            maxx = delta[i][T2-1]
            q_star[T2-1] = i
...
for file in range(file_count):
    ...
    # q_star = Viterbi(eval_seq2, A, B, pi, T, T2)
    #
    # output_seq = np.zeros(len(q_star)-2, dtype='<U1')
    # for i in range(len(output_seq)):
    #     output_seq[i] = convert_NumToChar(seq_type, int(q_star[i+1]))
